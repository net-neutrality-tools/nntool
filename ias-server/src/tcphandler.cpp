...
CTcpHandler::CTcpHandler(int nSocket, string nClientIp, bool nTlsSocket,
                         sockaddr_in6 *pClient) {
  mClient = pClient;
  mSocket = nSocket;
...
int CTcpHandler::handle_tcp() {
  ...
  noPollConn *conn =
      nopoll_listener_from_socket(ctx, (NOPOLL_SOCKET)mSocket, mTlsSocket);
  ...
  if (mTlsSocket) {
    ...
    if ((dir = opendir(certDir.c_str())) != NULL) {
      while ((ent = readdir(dir)) != NULL) {
        ...
        if (!dirName.compare(".") == 0 && !dirName.compare("..") == 0) {
          ...
          if (nopoll_ctx_set_certificate(ctx, dirName.c_str(), certFile.c_str(),
                                         keyFile.c_str(), NULL)) {
            TRC_DEBUG(
                "WebSocket handler: noPoll context TLS certificate set for: " +
                dirName);
          } else {
            TRC_CRIT(
                "WebSocket handler: error: noPoll context TLS certificate set "
                "failed for: " +
                dirName);
            return 0;
          }
      ...
      TRC_CRIT(
          "WebSocket handler: error: failed to open TLS certificate "
          "Directory: " +
          certDir);
      return 0;
    }
    ...
    nopoll_conn_accept_complete(ctx, conn, conn, (NOPOLL_SOCKET)mSocket,
                                nopoll_true);
  }
...
int CTcpHandler::websocket_open_handler(noPollCtx *ctx, noPollConn *conn,
                                        noPollPtr user_data) {
  ...
  for (vector<string>::iterator itAllowedProtocols = allowedProtocols.begin();
       itAllowedProtocols != allowedProtocols.end(); ++itAllowedProtocols) {
    for (vector<string>::iterator itRequestedProtocols =
             requestestedProtocols.begin();
         itRequestedProtocols != requestestedProtocols.end();
         ++itRequestedProtocols) {
      ...
      if (requestedProtocol.compare(*itAllowedProtocols) == 0) {
        if (requestestedProtocols.size() > 2) {
          ...
          TRC_DEBUG("WebSocket handler: requested protocol:               \"" +
                    requestedProtocol + "\"");
          TRC_DEBUG("WebSocket handler: requested token:                  \"" +
                    authToken + "\"");
          TRC_DEBUG("WebSocket handler: requested timestamp:              \"" +
                    authTimestamp + "\"");

          if (acceptedProtocol.compare("download") == 0 &&
              ((i == 0 && requestestedProtocols.size() > 3) ||
               (i == 1 && requestestedProtocols.size() > 4))) {
            downloadFrameSize = atoi(requestestedProtocols.at(i + 3).c_str());
            TRC_DEBUG(
                "WebSocket handler: requested download frame size:      \"" +
                to_string(downloadFrameSize) + "\"");
          }
  ...
  if (protocolAllowed) {
    ...
    if (overloadCheck && OVERLOADED) {
      ...
      TRC_DEBUG("WebSocket handler: requested protocol: \"" + acceptedProtocol +
                "\" is allowed");
    }
  } else if (!protocolAllowed) {
    TRC_ERR("WebSocket handler: requested protocol: \"" + sProtocol +
            "\" is not allowed or authorization failed");
...
int CTcpHandler::websocket_ready_handler(noPollCtx *ctx, noPollConn *conn,
                                         noPollPtr user_data) {
  TRC_DEBUG("WebSocket handler: ready for IP: " + sClientIp +
            " on Port: " + string(nopoll_conn_port(conn)));
...
int CTcpHandler::websocket_reject_handler(noPollCtx *ctx, noPollConn *conn,
                                          noPollPtr user_data) {
  ...
  if ((http_header_values["request_url"].string_value().compare(HTTP_DATA) ==
           0 &&
    ...
    TRC_INFO("WebSocket handler: valid HTTP " +
             http_header_values["http_method"].string_value() +
             " request received, falling back to HTTP");

    return handle_http(http_header_values, ctx, conn, user_data);
  } else {
    TRC_ERR("WebSocket handler: invalid HTTP " +
            http_header_values["http_method"].string_value() +
            " request received, closing connection");
...
int CTcpHandler::handle_http(Json::object http_header_values, noPollCtx *ctx,
                             noPollConn *conn, noPollPtr user_data) {
  ...
  if (cookies.size() > 1) {
    ...
    for (vector<string>::iterator itCookies = cookies.begin();
         itCookies != cookies.end(); ++itCookies) {
      cookie = *itCookies;
    ...
    TRC_DEBUG("HTTP handler: requested protocol:               \"HTTP " +
              http_header_values["http_method"].string_value() + "\"");
    TRC_DEBUG("HTTP handler: requested token:                  \"" + authToken +
              "\"");
    TRC_DEBUG("HTTP handler: requested timestamp:              \"" +
              authTimestamp + "\"");
    ...
    if (auth) {
      ...
      TRC_DEBUG("HTTP handler: ready for IP: " + sClientIp +
                " on Port: " + string(nopoll_conn_port(conn)));

      string response = "";

      if (http_header_values["http_method"].string_value().compare("GET") ==
          0) {
        response += "HTTP/1.1 200 OK\r\n";
        response += "Accept-Ranges: bytes\r\n";
        if (http_header_values["Origin"].string_value().compare("") != 0) {
          response += "Access-Control-Allow-Origin: " +
                      http_header_values["Origin"].string_value() + "\r\n";
        }
        ...
        nopoll_conn_default_send(conn, const_cast<char *>(response.c_str()),
                                 response.size());

        thread downloadThread(download, ctx, conn);
        downloadThread.detach();
      } else if (http_header_values["http_method"].string_value().compare(
                     "POST") == 0) {
      ...
      TRC_ERR("HTTP handler: requested protocol: \"HTTP " +
              http_header_values["http_method"].string_value() +
              "\" is not allowed or authorization failed");
...
void CTcpHandler::websocket_message_handler(noPollCtx *ctx, noPollConn *conn,
                                            noPollMsg *msg,
                                            noPollPtr user_data) {
  ...
  if (nopoll_msg_opcode(msg) == NOPOLL_PONG_FRAME && rttRunning) {
    ...
    if ((rttPayloadReceivedPos = rttPayloadReceivedString.find(
             rttPayloadDelimiter)) != string::npos) {
      TRC_DEBUG("WebSocket handler: rtt PONG received");

      string rttPayloadReceivedTimestamp =
          rttPayloadReceivedString.substr(0, rttPayloadReceivedPos);
  ...
  if (rttRunning && !rttStart) {
    string error;
    Json rttParameters =
        Json::parse((const char *)nopoll_msg_get_payload(msg), error);

    if (rttParameters["cmd"].string_value().compare("rttStart") == 0) {
      if (rttParameters["rttRequests"].int_value())
        rttRequests = rttParameters["rttRequests"].int_value() + 1;
      if (rttParameters["rttRequestTimeout"].int_value())
        rttRequestTimeout = rttParameters["rttRequestTimeout"].int_value();
      if (rttParameters["rttRequestWait"].int_value())
        rttRequestWait = rttParameters["rttRequestWait"].int_value();
      if (rttParameters["rttTimeout"].int_value())
        rttTimeout = rttParameters["rttTimeout"].int_value();
  ...
  TRC_DEBUG("SIZE RECEIVED: " + to_string(nopoll_msg_get_payload_size(msg)) +
  " OPCODE: " + to_string(nopoll_msg_opcode(msg)));
...
void CTcpHandler::websocket_close_handler(noPollCtx *ctx, noPollConn *conn,
                                          noPollPtr user_data) {
...
nopoll_bool CTcpHandler::websocket_post_ssl_handler(noPollCtx *ctx,
                                                    noPollConn *conn,
                                                    noPollPtr SSL_CTX,
                                                    noPollPtr SSL,
                                                    noPollPtr user_data) {
...
void CTcpHandler::nopoll_logging_handler(noPollCtx *ctx, noPollDebugLevel level,
                                         const char *log_msg,
                                         noPollPtr user_data) {
...
int CTcpHandler::roundTripTime(noPollCtx *ctx, noPollConn *conn) {
  ...
  do {
    ...
    if (nopoll_conn_send_frame(conn, nopoll_true, nopoll_false,
                               NOPOLL_PING_FRAME, rttPayloadSize,
                               (noPollPtr)rttPayload.c_str(), 0) == -1) {
      // PING send failed
      rttErrors++;
    ...
    while (rttPongReceiveTime == 0) {
      if (((CTool::get_timestamp() - rttPingSendTime) / 1000) >
          rttRequestTimeout) {
  ...
  } while (((runningTime / 1000) < rttTimeout) && rttRunning &&
           rttRequestsSend < rttRequests);
...
void CTcpHandler::setRoundTripTimeKPIs() {
  ...
  if (rttSum != 0 && rttReplies > 0) {
    ...
    if (length % 2 == 0) {
      rttMed =
          (rttMedianVector[length / 2 - 1] + rttMedianVector[length / 2]) / 2;
    } else {
    ...
    double variancePopulation =
        (double)rttSumSq / (double)rttReplies - rttAvg * rttAvg;
...
void CTcpHandler::sendRoundTripTimeResponse(noPollCtx *ctx, noPollConn *conn) {
  ...
  if (((rttRequestsSend - 1) % 2 == 1) || (rttRequestsSend == rttRequests)) {
    ...
    nopoll_conn_send_text(conn, rttReport.dump().c_str(),
                          rttReport.dump().length());
...
int CTcpHandler::download(noPollCtx *ctx, noPollConn *conn) {
  if (connectionIsValidWebSocket) {
    TRC_DEBUG("TCP handler: download using WebSocket, frame size: " +
              to_string(downloadFrameSize));
  ...
  do {
    ...
    if (connectionIsValidWebSocket) {
      nResponse =
          nopoll_conn_send_binary(conn, firstChar + index, downloadFrameSize);
    } else if (connectionIsValidHttp) {
      nResponse =
          nopoll_conn_default_send(conn, firstChar + index, downloadFrameSize);
    }
...
int CTcpHandler::upload(noPollCtx *ctx, noPollConn *conn) {
  ...
  do {
    ...
    if ((endTime - (currentTime * 100000)) > 500000) {
      ...
      // store the amount of bytes received in the actual and the previous
      // timeframe
      sResponse = "" + CTool::toString(uploadBytesReceived) + "," +
                  CTool::toString(uploadBytesReceivedLast) + "," +
                  CTool::toString(CTool::get_timestamp_sec()) + "," +
                  CTool::toString(currentTime) + "," +
                  CTool::toString(uploadHeaderReceived) + ";";
      ...
      if (connectionIsValidWebSocket) {
        ...
        nopoll_conn_default_send(conn, const_cast<char *>(sResponse.c_str()),
                                 sResponse.size());
      }
...
unsigned long long CTcpHandler::formatCurrentTime(
    unsigned long long endTime, unsigned long long currentTime) {
...
bool CTcpHandler::checkAuth(string authToken, string authTimestamp,
                            string handler) {
  ...
  if (((currentTimestamp - requestedTimestamp) >
       (AUTHENTICATION_MAX_AGE * 1e6)) ||
      ((currentTimestamp - requestedTimestamp) < 0)) {
    TRC_WARN("WebSocket handler: authentication failed: token expired: " +
             to_string((currentTimestamp - requestedTimestamp)));
    return false;
  }
...
void CTcpHandler::printTcpMetrics() {
  ...
  if (getsockopt(mSocket, SOL_TCP, TCP_INFO, &tcp_info, &tcp_info_length) !=
      0) {
    ...
    TRC_DEBUG(
        "############ TCP Metrics:\r\n"
        "   tcpi_pmtu:              " +
        to_string(tcp_info.tcpi_pmtu) +
        "\r\n"
        "   tcpi_rcv_ssthresh:      " +
        to_string(tcp_info.tcpi_rcv_ssthresh) +
        "\r\n"
        "   tcpi_rtt:               " +
        to_string(tcp_info.tcpi_rtt) +
        "\r\n"
        "   tcpi_rttvar:            " +
        to_string(tcp_info.tcpi_rttvar) +
        "\r\n"
        "   tcpi_snd_ssthresh:      " +
        to_string(tcp_info.tcpi_snd_ssthresh) +
        "\r\n"
        "   tcpi_snd_cwnd:          " +
        to_string(tcp_info.tcpi_snd_cwnd) +
        "\r\n"
        "   tcpi_rcv_rtt:           " +
        to_string(tcp_info.tcpi_rcv_rtt) +
        "\r\n"
        "   tcpi_rcv_space:         " +
        to_string(tcp_info.tcpi_rcv_space) +
        "\r\n"
        "   tcpi_total_retrans:     " +
        to_string(tcp_info.tcpi_total_retrans) +
        "\r\n"
        "   tcpi_options:           " +
        to_string(tcp_info.tcpi_options) +
        "\r\n"
        "   tcpi_snd_mss:           " +
        to_string(tcp_info.tcpi_snd_mss) +
        "\r\n"
        "   tcpi_advmss:            " +
        to_string(tcp_info.tcpi_advmss) +
        "\r\n"
        "   tcpi_rcv_mss:           " +
        to_string(tcp_info.tcpi_rcv_mss) + "");
  }
}
