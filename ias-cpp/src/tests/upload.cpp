...
int Upload::run()
{
    try {
        ...
        std::unique_ptr<CHttp> pHttp = std::make_unique<CHttp>( mConfig, mConnection, mUploadString );
        ...
        std::unique_ptr<CUploadSender> pUploadSender = std::make_unique<CUploadSender>(mConnection);
