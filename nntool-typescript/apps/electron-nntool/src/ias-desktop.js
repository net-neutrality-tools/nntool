...
IasDesktop.prototype.measurementStart = function(iasParameters) {
  ...
  tool.getInterfaceConfiguration().then(success => {
    if (typeof networkKPIs !== 'undefined') {
      ...
      if (typeof interface.speed !== 'undefined' && !isNaN(interface.speed))
        networkKPIs.dsk_interface_speed = interface.speed;
...
function iasCallback(data) {
  ...
  if (data.cmd === 'completed' || data.cmd === 'error') {
    ...
    if (data.server !== '-' && !routeToTargetCalled && !routeToClientCalled) {
      ...
      tool.getRouteToTarget(
        measurementParameters.wsTargets[Math.floor(Math.random() * measurementParameters.wsTargets.length)] +
          '.' +
          measurementParameters.wsTLD,
        routeToTargetMaxHops,
        measurementParameters.ndServerFamily
      );

      routeToClientCalled = true;
      if (typeof performRouteToClientLookup !== 'undefined' && performRouteToClientLookup) {
        jsTool.performRouteToClientLookup(
          measurementParameters.wsTargets[Math.floor(Math.random() * measurementParameters.wsTargets.length)] +
            '.' +
            measurementParameters.wsTLD,
          routeToClientTargetPort
        );
      } else {
      ...
      if (
        typeof networkKPIs.dsk_interface_ipv4_gateway !== 'undefined' &&
        networkKPIs.dsk_interface_ipv4_gateway !== '-'
      ) {
    ...
    //wait for completion
    if (
      (!rttToTargetCallbackCalled || !routeToTargetCallbackCalled || !routeToClientCallbackCalled) &&
      callbackWaitTime < callbackTimeout
    ) {
      console.log(
        'Measurement completed, waiting for completion of route client -> server, route server -> client and rttToTarget'
      );
      console.log(
        'rtt: ' +
          rttToTargetCallbackCalled +
          ' routeToTarget: ' +
          routeToTargetCallbackCalled +
          ' routeToClient: ' +
          routeToClientCallbackCalled
      );
      callbackWaitTime += 2000;
      setTimeout(iasCallback, 2000, JSON.stringify(data));
      return;
    } else if (callbackWaitTime >= callbackTimeout) {
...
function systemUsageCallback(data) {
  ...
  if (
    typeof systemUsage.dsk_cpu_load_avg !== 'undefined' &&
    !isNaN(systemUsage.dsk_cpu_load_avg) &&
    systemUsage.dsk_cpu_load_avg <= 1
  )
    systemUsageKPIs.dsk_cpu_load_avg = systemUsage.dsk_cpu_load_avg;
  if (
    typeof systemUsage.dsk_cpu_load_max !== 'undefined' &&
    !isNaN(systemUsage.dsk_cpu_load_max) &&
    systemUsage.dsk_cpu_load_max <= 1
  )
    systemUsageKPIs.dsk_cpu_load_max = systemUsage.dsk_cpu_load_max;
  if (
    typeof systemUsage.dsk_cpu_load_max_core !== 'undefined' &&
    !isNaN(systemUsage.dsk_cpu_load_max_core) &&
    systemUsage.dsk_cpu_load_max_core <= 1
  )
    systemUsageKPIs.dsk_cpu_load_max_core = systemUsage.dsk_cpu_load_max_core;
  if (
    typeof systemUsage.dsk_mem_load_avg !== 'undefined' &&
    !isNaN(systemUsage.dsk_mem_load_avg) &&
    systemUsage.dsk_mem_load_avg <= 1
  )
    systemUsageKPIs.dsk_mem_load_avg = systemUsage.dsk_mem_load_avg;
  if (
    typeof systemUsage.dsk_mem_load_max !== 'undefined' &&
    !isNaN(systemUsage.dsk_mem_load_max) &&
    systemUsage.dsk_mem_load_max <= 1
  )
    systemUsageKPIs.dsk_mem_load_max = systemUsage.dsk_mem_load_max;
}
