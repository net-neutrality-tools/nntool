...
export class MeasurementGauge extends BaseMeasurementGauge {
    ...
    private drawTextAlongArc(
        text: string, center: Point, radius: number, startRotate: number, maxAngle: number, factor: number,
        font: string, align: 'LEFT' | 'CENTER' | 'RIGHT'
    ...
    private drawTextList(
        ...
        for (const text of texts) {
            ...
            this.drawTextAlongArc(
                text, center, radius, angle, this.deg2rad(Math.floor(angleStep)), factor,
                font, align
            );
            i++;
        }
