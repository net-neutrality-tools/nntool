...
import {BriefMeasurementResponseAPI, BriefSubMeasurement} from '../test/models/results/brief-measurement-response.api';
...
export class HistoryComponent extends ResultListComponent implements OnInit {
        ...
        this.graph = {
            ...
            options: {
                chart: {
                    ...
                    clipEdge: true,
                    // yDomain: [0, 100],
                    x(d: any) { if (d) { return d.x; } },
                    y(d: any) { if (d) { return d.y; } },
                    showValues: true,
                    // duration: 500,
                    xAxis: {
                        showMaxMin: false,
                    ...
                    tooltip: {
                        contentGenerator: (d: any) => {
                            ...
                            return '<div style=\'color: ' + d.point.color + '\'>' +
                                '<h3>' + key + '</h3>' +
                                '<h4>' + d.point.y + unit + '</h4>' +
                                '</div>';
                        }
                    },
                }
    ...
    private initLang(): void {
        this.subs.push(this.translationService.get([
            'WEBSITE.UNITS.SPEED_MBPS', 'HISTORY.GRAPH.DOWNLOAD', 'HISTORY.GRAPH.UPLOAD'
        ]).subscribe(
            (res: any) => {
    ...
    private maxMinGraph(nvD3: NvD3Component): void {
        ...
        setTimeout(() => { nvD3.chart.update(); }, 0);
    }
        ...
        setTimeout(() => {
            if (this.nvD3 && this.nvD3) {
                ...
                this.nvD3.chart.legend.dispatch.on('legendClick', () => {
                    setTimeout(() => { this.maxMinGraph(this.nvD3); }, 0);
                });
    ...
    protected searchExecute(more: boolean): void {
        ...
        this.userService.loadMeasurements(user)
