...
export class AppComponent implements OnInit, OnDestroy {
    ...
    ngOnInit () {
        ...
        this.subs.push(this.translateService.onTranslationChange.subscribe(
        ...
        this.subs.push(this.router.events.pipe( filter((event: any) => event instanceof NavigationEnd) ).subscribe(
            (event: NavigationEnd) => {
                ...
                const url_location: string = this.location.normalize(
                    uri.split('#')[0].split('?')[0]
                ).slice(1).split('/')[0];
                if (url_location) {
                    let i: number = 0;
