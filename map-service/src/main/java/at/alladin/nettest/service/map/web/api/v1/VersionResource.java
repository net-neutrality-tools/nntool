...
public class VersionResource {
	 ...
	 * Returns the software version of the map server.
	 * 
	 * @return
	 */
	@io.swagger.annotations.ApiOperation(value = "Get the server version.", notes = "Returns the software version of the map server.")
    ...
    public ResponseEntity<ApiResponse<VersionResponse>> getVersion() {
		...
		versionResponse.setMapServiceVersion("0.0.0");
		
		//return ResponseHelper.ok(versionResponse);
		return new ResponseEntity<>(new ApiResponse<VersionResponse>(versionResponse), HttpStatus.OK);
	}
}
