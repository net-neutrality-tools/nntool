...
import at.alladin.rmbt.shared.qos.testscript.TestScriptInterpreter.EvalResult;
...
public class ResultComparer {
	...
	private static int runCompare(String operator, boolean controlFlag, Object result, Object expectedResult) {
		try {
			//as results may inherit fields we want to compare, compare the fields of all superclasses up to the AbstractResult
			Class<?> currentClass = result.getClass();
			while (!currentClass.equals(MAX_COMPARING_CLASS)) {
		...
		try {
			...
			//if the field is not a collection it can (hopefully) be compared
				...
				case AbstractResult.COMPARATOR_NOT_EQUALS:
					...
					//System.out.println("comparing " + leftPart + " to " + rightPart);
