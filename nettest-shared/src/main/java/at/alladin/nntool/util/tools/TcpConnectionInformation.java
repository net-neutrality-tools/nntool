 ...
 ******************************************************************************/
// based on: https://raw.githubusercontent.com/alladin-IT/open-rmbt/master/RMBTUtil/src/main/java/at/alladin/rmbt/util/tools/TcpConnectionInformation.java
...
public class TcpConnectionInformation extends ConnectionInformation {
	...
	public static TcpConnectionInformation parseAndroid(String str, int addrSize) {
		final TcpConnectionInformation conn = new TcpConnectionInformation();

		final String[] token = str.trim().replaceAll(" +", " ").split(" ");
		if (token.length < 12) {
			return null;
		}
