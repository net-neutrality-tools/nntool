...
import at.alladin.nntool.util.net.udp.UdpStreamReceiver;
import at.alladin.nntool.util.net.udp.UdpStreamReceiver.UdpStreamReceiverSettings;
		...
		try {
		    ...
		    try {
							...
							try {
								if (request.startsWith("GET UDPPORT")) {
									...
									if (response != null && response.startsWith("OK")) {
										...
										Future<UdpPacketData> udpOutTimeoutTask = ClientHolder.getCommonThreadPool().submit(new Callable<UdpPacketData>() {
				...
				if (this.packetCountIncoming != null && this.incomingPort != null) {
					...
					Future<UdpPacketData> udpInTimeoutTask = ClientHolder.getCommonThreadPool().submit(new Callable<UdpPacketData>() {
	   		...
	   		if (this.packetCountOutgoing != null) {
	   			...
	   			if (outgoingPacketData.rtts != null && outgoingPacketData.rtts.size() > 0) {
